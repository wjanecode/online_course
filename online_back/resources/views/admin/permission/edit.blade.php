            ...
            <div class="tabCon">
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-9">
                        <input type="text" class="input-text form-control @error('name') is-invalid @enderror" value="{{ $permission->name ?? old('name') }}" placeholder="" id="name" name="name">
                        @error('name')
                ...
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-9">
                        <select name="pid" id="">
                            {{-- 如果不是父级,显示父级和顶级选项--}}
                            @if($permission->pid != 0)
                                <option value="{{ $permission->pid }}" selected>{{ $permission->parent->name }}</option>
                ...
                <div class="row cl">
                    <label class="form-label col-xs-4 col-sm-3">
                        ...
                        路由别名：</label>
                    <div class="formControls col-xs-8 col-sm-9">
                        <input type="text" class="input-text form-control @error('routename') is-invalid @enderror" value="{{ $permission->routename ?? old('routename') }}" placeholder="" id="routename" name="routename">
                        @error('routename')
                        <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
        ...
        $(document).ready(function () {
            ...
            $('#form-permission-edit').validate({
                ...
                submitHandler:function(form){
                    $(form).ajaxSubmit({
                        type: "PUT",
                        url: "{{ url("/permission/$permission->id") }}",
                        // data: {
                        // },
